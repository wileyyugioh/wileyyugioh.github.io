...
Note the plugins and the relative links. I'll explain those later.
...
Also we want to disable the `/all/` subdirectory, so we can use the redirect-from plugin to achieve this. We just need to edit `404.html`.
